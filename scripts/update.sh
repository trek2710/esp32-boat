...
# ============================================================================
DEFAULT_MSG="Round 77: three small follow-ups to the round-76 Main-display overhaul. (1) AWS readout added below BSPD inside the hull, using the same DEPTH-style subtitle/value/unit triple ('AWS' / value / 'kn') at offsets y=42/62/80 from widget centre. New main_pg.aws_lbl field, populated from s.aws in refreshMain. (2) Top '0' manual mirrored label reverted from white (round 75) back to black: the round-75 white was needed because the label sat on the black bow-zone wedge at radius ~205, but round 76 pushed the wedge out to the bezel (r_mod=0, radii 214..240); the manual labels at radius 188 are now well inside the wedge ring on white bg, so all 12 labels go back to black. (3) Apparent-wind cone (struct field twd_arrow, drives s.awa since round 76) tip pushed outward: kConeH 182→215 with kConeVisibleBase unchanged at 117, so the visible cone stays the same length — only the invisible pivot padding grows, translating the whole cone outward without rescaling. Tip-to-pivot distance is now 214, landing exactly on the wedge inner edge."
